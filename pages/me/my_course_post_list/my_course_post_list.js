...
Page({
  ...
  refreshPostList() {
    ...
    let data = {
      ...
      status: this.data.filterOnlyOpen ? 0 : 1
    }

    wxw.getMyCoursePost(app.globalData.session, data)
      .then(res => {
        console.log(res)
        let data = {
          start: res.length,
    ...
    wx.showActionSheet({
      ...
      success(res) {
        if (!res.cancel) {
          ...
          return wxw.editCoursePost(app.globalData.session, data, e.target.dataset.id)
            .then(res => {
