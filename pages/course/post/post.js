...
Page({
  ...
  refreshPost(id) {
    ...
    wxw.showLoading()
    wxw.getCoursePost(app.globalData.session, id)
      .then(res => {
        console.log(res)
  ...
  onLoad: function(options) {
    ...
    if (!options.id) {
      ...
      wxw.getCoursePostRemainingViewCount(app.globalData.session)
        .then(res => {
          that.setData({
  ...
  onShareAppMessage: function() {
    ...
    return {
      ...
      success(res) {
        wxw.coursePostShare(that.data.postId, 1, app.globalData.bindInfo.id)
      }
    }
  },
    ...
    if (!this.data.hasViewedContact) {
      wx.showModal({
        ...
        success(res) {
          if (res.confirm) {
            wx.showNavigationBarLoading()
            wxw.viewCoursePostContact(app.globalData.session, that.data.postId)
              .then(res => {
                console.log(res)
                that.setData({
  ...
  sharePost(e) {
    ...
    wx.showModal({
      ...
      complete() {
        wxw.showLoading()
        wxw.getSharedCoursePostImage(that.data.post, that.data.bindInfo.id)
          .then(res => {
            wxw.hideLoading()
            wx.previewImage({
    ...
    wx.showActionSheet({
      ...
      success(res) {
        if (!res.cancel) {
          ...
          wxw.editCoursePost(app.globalData.session, data, that.data.postId)
            .then(res => {
              that.refreshPost(that.data.postId)
