...
Page({
  ...
  refreshPostList() {
    ...
    wxw.getGoodsPostList(app.globalData.session, this.data.order, 0, this.data.limit, this.data.filterOnlyOpen ? 0 : null)
      .then(res => {
        let data = {
          start: res.length,
          loading: false,
  ...
  onReachBottom(e) {
    ...
    if (that.data.hasMore && !this.data.loading) {
      ...
      wxw.getGoodsPostList(app.globalData.session, this.data.order, this.data.start, this.data.limit, this.data.filterOnlyOpen ? 0 : null)
        .then(res => {
          let data = {
